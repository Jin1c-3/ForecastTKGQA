...
class TemporalPathAgg(MessagePassing):
    ...
    def forward(self, input, query, initial_stat, edge_index, edge_type, edge_time, size, edge_weight=None):
        ...
        output = self.propagate(edge_index=edge_index,size=size, input=input, relation=relation, initial_stat=initial_stat,
                                edge_type=edge_type, edge_time=edge_time, edge_weight=edge_weight)
        return output

...
# Code borrowed from TANGO official repository
# Modified for saving TANGO representations
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='ICEWS21', help='dataset name')
    ...
    ################################################
    # ts2emb is a dictionary to store TANGO representations at every timestamp during one time inference
    # Mentioned in the section "Temporal Knowledge Graph Forecasting Model" of the submission paper
    # ts2emb is in the form of {ts: emb[ts]}
    ts2emb = {}
    dict_path = modelpth + 'submission/tango_submission.pkl' # this is the saving path of ts2emb
    ################################################
    print(dict_path)
    ...
    if args.induct_test:
        ...
        train_dataset = TANGOtrainDataset(args,
                                           ...
                                           neg_samp=False)
        train_dataset2 = TANGOtestDataset(args,
                                           train_triple,
                                           train_adj,
                                           train_adjmtx,
                                           train_so2r,
        ...
        train_loader = TANGOtrainDataLoader(dataset=train_dataset,
                                  batch_size=1,
                                  shuffle=args.shuffle)
        train_loader2 = TANGOtestDataLoader(dataset=train_dataset2,
                                  batch_size=1,
                                  shuffle=args.shuffle)
        test_loader = TANGOtestDataLoader(dataset=test_dataset,
                                 batch_size=1,
    ...
    if args.induct_test == False and args.test == False:
        for epoch in range(args.num_epoch):
            ...
            if (epoch+1) % args.test_step == 0:
                if val_exist:
                    split = 'val'
                    results, _ = predict(val_loader, model, args, num_e, test_adjmtx, logger, ts2emb)
                else:
                    split = 'test'
                    results, _ = predict(test_loader, model, args, num_e, test_adjmtx, logger, ts2emb)
        ...
        # One time inference
        # Mentioned in the section "Temporal Knowledge Graph Forecasting Model" of the submission paper
        # pre_emb is the trained TANGO entity & relation representation
        pre_emb = torch.cat([model.emb_e, model.emb_r], dim=0)
        # print(pre_emb.detach().cpu().numpy())

        # Load pre_emb to infer the TANGO representations as every timestamp and store into ts2_emb
        results, ts2emb_result = predict(val_loader, model, args, num_e, val_adjmtx, logger, ts2emb)
        results, ts2emb_result = predict(test_loader, model, args, num_e, test_adjmtx, logger, ts2emb_result)
        results, ts2emb_result = predict(train_loader2, model, args, num_e, train_adjmtx, logger, ts2emb_result)

        # First 4 timestamp has no TANGO information, we inititialize them as the pre_emb
        ts2emb_result.update({0: pre_emb,
                              24: pre_emb,
                              48: pre_emb,
                              72: pre_emb})
        # Save TANGO representations
        # NOTE that ts2emb is very huge
        with open(dict_path, 'wb') as fp:
            pickle.dump(ts2emb_result, fp)
        ####################################
        print("===========RAW===========")
        print("Epoch {}, HITS10 {}".format(epoch + 1, results['hits@10_raw']))
        print("Epoch {}, HITS3 {}".format(epoch + 1, results['hits@3_raw']))

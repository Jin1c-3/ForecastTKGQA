...
def eval_model(
    qa_model, dataloader, dataset, split, batch_size, device
):  # Evaluation code
    ...
    if ddputils.should_execute():
        eval_log.append(f"Split {split}")
        eval_log.append("Evaluation Result.")
    print(f"Evaluating {split} on device {device}")
    ...
    # Dataloader
    loader = (
        tqdm(dataloader, total=len(dataloader), unit="batches")
        if device == 0
        else dataloader
    )
    ...
    with torch.no_grad():
        for i_batch, (
            ...
            #     break
            mask_dis, scores_ep, scores_yn, scores_mc = qa_model.forward(
                question_tokenized,
            ...
            if none_count <= 1 or none_count == 3:
                raise ValueError(f"得分异常，{scores_ep=}, {scores_yn=}, {scores_mc=}")
            ...
            if scores_ep is not None:  # If the question type is entity prediction
                ...
                score_vector = scores_ep[
                    ind_batch, answers.squeeze()
                ].clone()  # scores for right answer
                ...
                for text_idx in batch_sentences:  # a[-1] sentences list for indexing
                    ...
                    filter[filter_idx, answers_multiple] = (
                        0.5  # index data in batch, change data in corresponding position from zero to a half
                    )
                    filter_idx += 1
                ...
                scores_ep = torch.where(
                    filter > 0, torch.tensor(-100000.0).cuda(device), scores_ep
                )
    ...
    if truth_entities:  # If the question type is entity prediction
        ...
        mrr, mrr_1_hop, count_1_hop, mrr_2_hop, count_2_hop = ddputils.reduce_scalars(
            torch.tensor(mrr),
            torch.tensor(mrr_1_hop),
            torch.tensor(count_1_hop),
            torch.tensor(mrr_2_hop),
            torch.tensor(count_2_hop),
            average=False,
        )
        hop_types_bytes_data = list(("@".join(hop_types) + "@").encode("utf-8"))
        truth_entities, predicted_answers, types, hop_types_bytes_data = (
            ddputils.gather_tensors(
                torch.tensor(truth_entities),
                torch.tensor(predicted_answers),
                types,
                torch.ByteTensor(hop_types_bytes_data),
            )
        )
        mrr, mrr_1_hop, count_1_hop, mrr_2_hop, count_2_hop = (
            item.item()
            for item in [mrr, mrr_1_hop, count_1_hop, mrr_2_hop, count_2_hop]
        )
        hop_types = [
            h
            for h in hop_types_bytes_data.numpy().tobytes().decode("utf-8").split("@")
            if h
        ]
        truth_entities, predicted_answers = (
            item.tolist() for item in [truth_entities, predicted_answers]
        )
        ...
        eval_hits(
            truth_entities, hop_types, predicted_answers, eval_log
        )  # Hits@k computation
    if truth_class:  # If the question type is yes-no
        # aggregate distributed data
        # predicted_class=ddputils.gather_tensors(torch.tensor(predicted_class)).tolist()
        # truth_class = ddputils.gather_tensors(truth_class)

        score_clf = eval_acc(
            predicted_class, truth_class, "yes_no", eval_log
        )  # Accuracy computation
    if truth_choice:  # If the question type is fact reasoning
        # aggregate distributed data
        # predicted_choice=ddputils.gather_tensors(torch.tensor(predicted_choice)).tolist()
        # truth_choice = ddputils.gather_tensors(truth_choice)

        score_choice = eval_acc(
            predicted_choice, truth_choice, "multiple_choice", eval_log
        )  # Accuracy computation
    ...
    # 返回值都是正确聚合后的，其他进程的eval_log应该是空的
...
def eval_acc(predicts, truths, question_type, eval_log):  # Accuracy computation
    ...
    prediction = np.argmax(
        prediction, axis=1
    ).flatten()  # Find the candidate with the highest score
    truth = np.concatenate(
        [t.cpu() for t in truths],
    )
    test_accuracy = sum(np.array(prediction) == np.array(truth)) / float(len(truth))
    eval_log.append(
        f"accuracy for {question_type} {round(test_accuracy, 5)} \t total questions: {len(truth)}"
    )
...
def record_hits(results, eval_log):
    ...
    for key, value in results.items():
        ...
        s = "{q_type} \t {hits_at_k} \t total questions: {num_questions}".format(
            q_type=key, hits_at_k=round(hits_at_k, 3), num_questions=len(value)
        )
        eval_log.append(s)

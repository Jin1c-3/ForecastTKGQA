...
def main():
    ...
    parser.add_argument(
        "--save_to", default="", type=str, help="Where to save checkpoint"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--dataset_name", default="ICEWS21", type=str, help="Which dataset"
    )
    parser.add_argument(
        "--question_type",
        default="all",
    ...
    # Path to save logs
    log_filename = f"results/{args.dataset_name}/{args.question_type}_{args.model_file}.log"
    if args.save_to == "":
        args.save_to = "temp"

    # Checkpoint name
    checkpoint_file_name = f"models/{args.dataset_name}/qa_models/{args.question_type}_{args.model_file}.ckpt"
    ...
    train_sampler = torch.utils.data.distributed.DistributedSampler(
        train_dataset, dist.get_world_size(), local_rank
    )
    ...
    print_if_rank_zero(
        f"info for training set: {train_dataset.get_dataset_ques_info()}"
    )
    valid_sampler = torch.utils.data.distributed.DistributedSampler(
        valid_dataset, dist.get_world_size(), local_rank
    )
    ...
    qa_model = torch.nn.parallel.DistributedDataParallel(
        qa_model, device_ids=[local_rank], find_unused_parameters=True
    )
    ...
    if args.train:
        ...
        for epoch in range(args.max_epochs):
            ...
            for i_batch, (
                ...
                question_attention_mask = b_attention_mask.cuda(
                ...
                qa_model.zero_grad()
                # Compute batch loss
                _, scores_ep, scores_yn, scores_mc = qa_model.forward(
                    question_tokenized,
                ...
                loader.set_postfix(
                    Loss=running_loss / ((i_batch + 1) * args.batch_size), Epoch=epoch
                )
            ...
            if (epoch + 1) % args.valid_freq == 0:
                ...
                _, _, choice_score_test, log = eval_model(
                    qa_model,
                    test_dataloader,
                    test_dataset,
                    "test",
    ...
    if args.eval:
        ...
        _, _, _, log = eval_model(
            qa_model, test_dataloader, test_dataset, "test", args.batch_size, local_rank
        )
